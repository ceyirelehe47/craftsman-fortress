...
该入口先完整重跑 R1 图形验收，再执行体素删除/放置、跨 Chunk 标脏、覆盖层规范化、双槽保存、精确加载、完整有效槽回退和槽位修复。实现方与隔离 Reviewer 都必须从全新克隆运行。

R2.1 起，“有效槽”不仅要求二进制格式和 FNV 校验通过，还必须能够重新生成基础世界、通过基础语义哈希、应用覆盖层并通过最终语义哈希。高 generation 槽在任一阶段失败时会被忽略，加载继续尝试旧槽。无窗口验收受 `R2_HEADLESS_WATCHDOG_SEC` 保护，损坏启动路径必须在创建窗口前以退出码 3 结束。

现有 `r2-baseline-3b5ae690e349` 只保留为历史候选轮；R2.1 通过后使用新的不可移动标签。
