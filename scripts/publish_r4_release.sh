#!/usr/bin/env bash
# 发布 R4 不可移动基线；发布后下载复验失败时撤销本轮新建的标签与 Release。
set -euo pipefail
cd "$(dirname "$0")/.."
export LC_ALL=C

S="${1:?usage: publish_r4_release.sh <S> <impl.zip> <review.zip> <review.md> <selection.tsv> <inventory.tsv>}"
...
for zip in "$IMPL" "$REVIEW"; do
  out="$TMP/$(basename "$zip" .zip)"
  mkdir -p "$out"
  unzip -q "$zip" -d "$out" || [ "$?" -eq 1 ]
...
git tag -a "$TAG" "$S" -m "R4 independent object layer baseline $S"
git push origin "$TAG"
CREATED_TAG=1
gh release create "$TAG" --repo "$REPO" \
  "$TMP/$IMPL_NAME" "$TMP/$REVIEW_NAME" "$TMP/SHA256SUMS.txt" \
  --title "R4 独立对象层、放置规则与对象存档（$SHORT）" \
  --notes "代码验收提交 S：\`$S\`\n\n实现方证据：\`$IMPL_NAME\`（SHA-256 \`$IMPL_SHA\`）\nReviewer 证据：\`$REVIEW_NAME\`（SHA-256 \`$REVIEW_SHA\`）\n\n第三方模型、贴图和原包未包含在 Release 中。"
CREATED_RELEASE=1
bash scripts/verify_r4_release.sh "$TAG" "$MANIFEST" "$INVENTORY"
trap - EXIT
rm -rf "$TMP"
echo "R4 Release 发布完成：$TAG"
