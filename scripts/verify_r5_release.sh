#!/usr/bin/env bash
# 下载复验 R5 标签、证据提交、F01-F12、校验和与第三方素材泄漏。
set -euo pipefail
cd "$(dirname "$0")/.."
export LC_ALL=C

TAG="${1:?usage: verify_r5_release.sh <tag> <selection.tsv> <inventory.tsv>}"
...
case "$TAG" in r5-baseline-*) ;; *) echo "失败：标签名不是 r5-baseline-*" >&2; exit 1;; esac
SHORT=${TAG#r5-baseline-}
[ "${TARGET:0:12}" = "$SHORT" ] || {
  echo "失败：标签短 SHA 与目标不一致：$TAG -> $TARGET" >&2
  exit 1
}
HIST=$(git rev-list -n 1 r4-baseline-1c05cfafce2e 2>/dev/null || true)
[ "$HIST" = '1c05cfafce2e9dabf46eb5853cd58ece1bf0bbdc' ] || {
  echo "失败：R4 历史标签缺失或移动。" >&2
  exit 1
}
git merge-base --is-ancestor "$HIST" "$TARGET" || {
  echo "失败：R5 标签目标不是 R4 正式基线的后代：$TARGET" >&2
  exit 1
}
...
for zip_file in "${zips[@]}"; do
  out="$TMP/unpacked_$(basename "$zip_file" .zip)"
  mkdir -p "$out"
  rc=0
  unzip -q "$zip_file" -d "$out" || rc=$?
  [ "$rc" -le 1 ] || { echo "失败：无法展开 $zip_file" >&2; exit 1; }
...
echo "R5 Release 验证通过：$TAG -> $TARGET"
