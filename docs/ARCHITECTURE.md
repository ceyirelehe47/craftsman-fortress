...
- **完整有效槽**：每个槽必须独立完成解码、基础世界生成、基础哈希、覆盖层应用和最终哈希；只有完整有效槽才参与 generation 排序。
- **崩溃恢复**：逻辑存档路径轮换两个不可原地覆盖的槽位；保存侧同样只把完整有效槽视为受保护旧状态。
...
`scripts/r2_acceptance.sh` 先调用 R1 图形验收，随后通过 `scripts/r2_headless_checks.sh` 在 watchdog 下运行存档往返、语义损坏回退、槽位修复与主程序损坏加载退出码检查。最终通过条件是机器可读 B01–B12、R1 回归和隔离 Reviewer 重跑全部 PASS。
